...
export function ComplianceCanvas({ content, fileName, ragResponse, searchQueries, documentCount, deepSearchResult: externalDeepSearchResult }: ComplianceCanvasProps) {
  ...
  // Append the A.M. No. 25-11-28-SC AI-use disclosure to every export (PRD P0-3).
  const buildExportContentWithDisclosure = (base: string): string => {
    const citedAuthorities: DisclosureAuthority[] = (ragResponse?.matched_documents ?? [])
      .map((doc) => ({ citation: doc.statute || doc.title, sourceUrl: doc.source_url ?? null }))
      .filter((entry) => Boolean(entry.citation))
    const disclosure = buildAiUseDisclosure({
      version: process.env.NEXT_PUBLIC_APP_VERSION ?? '0.5.2',
      providerMode: ragResponse?.provider_mode ?? 'local-providerless',
      reason: 'Philippine legal research and document compliance review',
      humanOversight: 'Draft generated by LexInsights; reviewed and edited by a qualified person before use',
      citedAuthorities,
      generatedAt: new Date().toISOString(),
    })
    return `${base}\n\n---\n\n${renderAiUseDisclosureMarkdown(disclosure)}`
  }
