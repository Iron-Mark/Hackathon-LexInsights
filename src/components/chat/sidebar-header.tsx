...
function HeaderTooltipButton({ label, children }: HeaderTooltipButtonProps) {
  return (
    <div className="group relative z-40">
      ...
      >
        {label}
        <span className="absolute right-3 top-[-5px] h-2.5 w-2.5 rotate-45 border-l border-t border-slate-200 bg-white dark:border-iris-300/15 dark:bg-[#241f32]" />
      </div>
    </div>
  )
}
...
export function SidebarHeader() {
  ...
  return (
    <header className="relative z-40 flex overflow-visible items-center justify-between border-b border-slate-200 bg-white/90 px-4 py-3 backdrop-blur dark:border-iris-300/15 dark:bg-[linear-gradient(135deg,#211a35_0%,#1a1625_48%,#171322_100%)] dark:shadow-[inset_0_-1px_0_rgba(158,151,227,0.10)]">
      {/* App Branding Area */}
      <div
        className="flex min-w-0 items-center gap-2.5"
        aria-label="LexInSight Legal compliance assistant"
      >
        <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-slate-200 bg-white p-1 shadow-sm shadow-slate-900/5 ring-1 ring-white transition-colors duration-200 dark:border-iris-300/15 dark:bg-iris-300/10 dark:shadow-[0_0_18px_rgba(63,51,189,0.16)] dark:ring-iris-100/10">
          <Image
      ...
      <div className="flex items-center gap-1">
        ...
        <HeaderTooltipButton label={isOpen ? 'Collapse sidebar' : 'Expand sidebar'}>
          <Button
            ...
            variant="ghost"
            size="icon"
            className="h-9 w-9 text-slate-600 transition-all duration-150 hover:bg-slate-100 hover:text-slate-900 active:scale-95 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-iris-400 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-50 dark:text-iris-100/75 dark:hover:bg-iris-300/12 dark:hover:text-white dark:focus-visible:ring-offset-[#171322]"
