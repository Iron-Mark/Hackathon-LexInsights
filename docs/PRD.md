...
*Budget note (2026-07-08):* with no paid Supabase and a free-tier 7-day inactivity pause, the near-term implementation is client-side durable storage (IndexedDB) behind the same `ComplianceReportRepository` interface, so reports survive reloads with no backend spend ([idb-storage.ts](../src/lib/store/idb-storage.ts), wired into [compliance-store.ts](../src/lib/store/compliance-store.ts)). The tradeoff is that IndexedDB is per-device, so cross-device sync and the server audit trail are deferred until budget allows; swapping in a `SupabaseComplianceReportRepository` is then a one-line factory change. A scheduled keep-alive ([.github/workflows/supabase-keepalive.yml](../.github/workflows/supabase-keepalive.yml)) calls the production PostgREST database API daily and fails visibly if the authenticated persistence backend is unavailable.

*Status (wired 2026-07-28):* the server path is now live for signed-in users as a dual-write. Every saved version mirrors to Supabase — the `compliance_reports` row is created on first save and kept at the latest content, each save appends an immutable `report_versions` row (version numbers derived server-side to respect `UNIQUE(report_id, version_number)` across tabs), and findings/checklist items persist to normalized `report_findings` rows extracted from the structured draft-checker analysis (markdown parsing as fallback for hand-edited saves; [findings-extractor.ts](../src/lib/services/compliance-persistence/findings-extractor.ts)). On sign-in with an empty local store, the latest server report and its version history rehydrate into the canvas, so a report now survives clearing browser storage ([sync.ts](../src/lib/services/compliance-persistence/sync.ts), [compliance-server-sync.ts](../src/lib/store/compliance-server-sync.ts)). IndexedDB remains the local copy and the only store for guests; a server failure never blocks a local save. Robustness choices from adversarial review: all server operations run through a serial queue (prevents duplicate report rows from concurrent first saves and delete-vs-save races), a stale server-report link recovers by recreating the row, the chat id is recorded in report metadata rather than the `chat_id` foreign key (that column is `ON DELETE CASCADE`, so FK-linking would let an ordinary chat deletion destroy the persisted report), scores are clamped to the DB's 0-100 CHECK, version-number collisions retry once with a server-derived number, and the migration's INSERT policies verify parent-report ownership. Prerequisites for the server side to accept writes: apply [0001_compliance_report_persistence.sql](../database/migrations/0001_compliance_report_persistence.sql) and configure Clerk as a Supabase third-party auth provider. Still open: retroactive sync of reports saved while signed out, checklist check-state persistence (the `setFindingChecked` repository method exists but no interactive UI consumes it yet), the retention-window decision (§11), and the pre-existing auth-blind local store caveat — on a shared browser, versions created by a guest remain visible to (and can be saved under) whoever signs in next on that device.
...
*Follow-up (shipped 2026-07-28):* the local research engine now emits an explicit `matched_framework_id` (plus `matched_framework_ids`, strongest first) on every completed response, computed from the same framework matches rendered in the report's "Local Compliance Framework" section ([local-legal-research.ts](../src/lib/services/local-legal-research.ts)). Framework detection treats the id as an authoritative signal — exact, high-confidence template selection — while unknown ids and absent fields (remote-RAG, cached, or pre-change persisted responses) degrade to the fuzzy inference above. The id also persists into chat message metadata and each server-persisted report's metadata (P0-1).
...
| Infra maturity | End-to-end Playwright only, Chromium only, no unit tests, no DB migration tooling | Add unit coverage and migrations alongside P0-1's schema work, not after. Update 2026-07-28: vitest landed with the P0-1 wiring (`npm run test`, gated in CI and `check:local`), covering the persistence mappers, findings extraction, sync orchestration, and framework detection |
