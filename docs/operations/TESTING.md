...
The local RAG answer-quality check runs realistic workflow prompts and asserts required authorities, forbidden-authority exclusions, answer fragments, unknown-citation handling, confidence floors, and HTTPS source metadata.
The local RAG source-freshness check validates corpus-to-source and corpus-to-coverage parity, trusted HTTPS source hosts, sane catalog or verification dates, and provenance-note coverage.
